...
Task ID: 16
Agent: main
Task: Фикс — пользователя выкидывало обратно на страницу входа после регистрации/логина

Work Log:
- Диагноз: в логах после успешного POST /api/auth/callback/credentials 200 и GET / иногда шёл GET /login — useSession() на главной получал unauthenticated, т.к. cookie сессии ещё не были сохранены браузером к моменту навигации (timing issue).
- Фикс в auth-screen.tsx: после signIn({redirect:false}) добавлен polling getSession() — до 20 попыток по 150мс (3 сек макс), пока сессия не появится. Только после подтверждения сессии — window.location.href="/".
- Также добавлена проверка result.ok (не только !result.error) для надёжности.
- Agent Browser E2E: 3 последовательных входа adrian@demo.app → все 3 приземлились на / (не /login) ✓; регистрация нового юзера → приземление на / с профилем LVL 0 ✓; в логах нет лишних GET /login после успешного входа ✓.
